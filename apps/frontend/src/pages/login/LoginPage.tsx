...
import { login, setAuthToken } from "../../shared/api/client";
  ...
  return (
    <main className="site-container grid min-h-[calc(100vh-140px)] place-items-center py-10">
      <section className="grid w-full max-w-[840px] grid-cols-[340px_minmax(0,1fr)] overflow-hidden rounded-sm bg-white shadow-card max-md:grid-cols-1">
        <div className="flex min-h-[520px] flex-col justify-between bg-brand-blue px-8 py-10 text-white">
          <div>
            <p className="mb-4 text-4xl font-semibold">Login</p>
            <p className="max-w-[220px] text-[1.15rem] leading-9 text-white/90">
        ...
        <form className="flex min-h-[520px] flex-col px-8 py-10" onSubmit={form.handleSubmit((values) => loginMutation.mutate(values))}>
          <div className="space-y-8">
            ...
            <label className="block">
              <span className="text-[15px] text-slate-500">Enter Password</span>
              <input
          ...
          {loginMutation.isError ? <p className="mt-5 text-sm font-semibold text-red-600">Login failed. Check your email and password.</p> : null}
